...
resource "null_resource" "enc_dbpasswd" {
  provisioner "local-exec" {
    command = <<EOT
     aws --profile ${var.profile} --region ${var.region} kms encrypt --key-id ${aws_kms_key.a.key_id} --plaintext ${var.db_passwd} --output text --query CiphertextBlob > /tmp/db_passwd_${var.region};
     aws --profile ${var.profile} --region ${var.region} kms encrypt --key-id ${aws_kms_key.a.key_id} --plaintext fileb://../../scripts/server.crt --output text --query CiphertextBlob > /tmp/server.crt_${var.region};
     aws --profile ${var.profile} --region ${var.region} kms encrypt --key-id ${aws_kms_key.a.key_id} --plaintext fileb://../../scripts/server.key --output text --query CiphertextBlob > /tmp/server.key_${var.region}
   EOT
  }
}
